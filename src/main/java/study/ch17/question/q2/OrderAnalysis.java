package study.ch17.question.q2;
...
public class OrderAnalysis {
  public static void main(String[] args) {
    ...
    // 결과: [1, 4]
    List<Integer> list1 = orders.stream()
        .filter(
            s -> s.getTotalAmount() > 1000000
        )
    ...
    Map<String, Double> map1 = orders.stream()
        .collect(
    ...
    // 결과: 전자제품
    orders.stream().flatMap(s -> s.getProducts().stream())
    ...
    List<String> list2 = orders.stream()
        .filter(s -> s.getOrderDate().isAfter(LocalDate.of(2023, 4, 30)))
    ...
    Map<String, Double> map2 = orders.stream()
        .flatMap(s -> s.getProducts().stream())
        .collect(
            Collectors.groupingBy(
                Product::getCategory,
    ...
    Map<LocalDate, Double> map3 = orders.stream()
        .collect(
            Collectors.groupingBy(
                Order::getOrderDate,
                Collectors.summingDouble(Order::getTotalAmount)
